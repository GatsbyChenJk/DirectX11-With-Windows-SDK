    ...
    float waveSpeed, float damping, float flowSpeedX, float flowSpeedY, bool cpuWrite)
{
    ...
    if (TextureManager::Get().GetTexture("..\\Texture\\water1.dds") == nullptr)
        TextureManager::Get().CreateFromFile("..\\Texture\\water1.dds", false, true);
    m_Model.materials[0].Set<std::string>("$Diffuse", "..\\Texture\\water1.dds");
...
std::unique_ptr<EffectHelper> FFTWaves::m_pEffectHelper = nullptr;
    ...
    float timeStep, float spatialStep, float waveSpeed, float damping, float flowSpeedX, float flowSpeedY)
{
    ...
    if (!m_pEffectHelper)
    {
        ...
        std::vector<LPCWSTR> fileName
        {
            ...
            L"Shaders/Update_CS.cso",
            L"Shaders/FFT_VitalCore_CS.cso",
            L"Shaders/FFT_Normal_CS.cso"
        };
...
void FFTWaves::FFTPrecompute(ID3D11DeviceContext* deviceContext, float L, float A, float G, float wind[2])
{
    m_L = L;
    m_A = A;
    m_G = G;
    ...
    auto pPass = m_pEffectHelper->GetEffectPass("FFTPrecompute");
    pPass->Apply(deviceContext);
    pPass->Dispatch(deviceContext, m_NumCols, m_NumRows);
...
void FFTWaves::FFTUpdate(ID3D11DeviceContext* deviceContext, float dt)
{
    ...
    pPass = m_pEffectHelper->GetEffectPass("FFTNormal");
    pPass->Apply(deviceContext);
    pPass->Dispatch(deviceContext, m_NumCols, m_NumRows);
