...
{
public:   
    enum class TessellationMode { Triangle, Quad, BezierCurve, BezierSurface, Torus };
...
private:
    ...
    ComPtr<ID3D11Buffer> m_pTorusVB;                                    // 甜甜圈曲面缓冲区
    D3D11_BUFFER_DESC TorusVBDesc;
    float m_TorusRadiusInside = 5.0f;
    float m_TorusRadiusOutside = 10.0f;
    float m_TorusPatchEdgeTess[4] = { 25.0f, 25.0f, 25.0f, 25.0f };		// 甜甜圈曲面各边的细分因子
    float m_TorusPatchInsideTess[2] = { 25.0f, 25.0f };					// 甜甜圈曲面内部的细分因子
    //float m_Phi = 3.14159f / 4, m_Theta = 0.0f, m_Radius = 30.0f;       // 摄像机参数
